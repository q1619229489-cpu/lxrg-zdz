...
const destinations = [
  { id: 1, name: "成都", desc: "火锅、熊猫、慢生活，主打一个巴适", tags: ["美食街", "休闲", "古镇"], traits: { action: 45, planning: 30, photo: 55, spontaneity: 65, food: 90, budget: 55 } },
  { id: 2, name: "西安", desc: "历史和肉夹馍爱好者的天堂", tags: ["博物馆", "历史遗迹", "美食街"], traits: { action: 70, planning: 65, photo: 50, spontaneity: 30, food: 80, budget: 55 } },
...
exports.main = async (event, context) => {
  try {
    ...
    console.log("join-matching query result count:", res.data.length, "for code:", inviteCode)

    if (res.data.length === 0) {
      return { code: -2, message: "邀请码无效或已过期", debug: { searchedCode: inviteCode } }
    }

    const record = res.data[0]

    const relationship = findRelationship(record.creator.personality, userResult.personality)
    const dests = recommendDestination(
      relationship,
    ...
    console.error("join-matching error:", e.message, e.stack)
    return { code: -99, message: "云函数异常: " + e.message }
  }
}
