...
export function calculateResult(answers) {
  var scores = {}
  personalities.forEach(function(p) { scores[p.id] = 0 })
  ...
  answers.forEach(function(selectedIndices, qIndex) {
    var question = allQuestions[qIndex]
    if (!question) return
    ...
    selections.forEach(function(idx) {
      ...
      if (!option) return
      if (scores[option.personality] !== undefined) scores[option.personality]++
      if (option.traits) {
        Object.keys(option.traits).forEach(function(t) {
          if (traits[t] !== undefined) traits[t] += option.traits[t]
        })
      }
      if (question.multi) {
        if (qIndex === 9) foodPrefs.push(option.personality)
        else if (qIndex === 19) destPrefs.push(option.personality)
        else if (qIndex === 29) destPrefs.push(option.personality)
      }
    })
  })
  ...
  Object.entries(scores).forEach(function(_ref) {
    ...
    if (score > maxScore) { maxScore = score; topPersonalities = [p] }
    else if (score === maxScore) { topPersonalities.push(p) }
  })
  ...
  return { personality: personality, scores: scores, maxScore: maxScore, traits: normalizedTraits, rawTraits: traits, foodPrefs: foodPrefs, destPrefs: destPrefs, pacePrefs: pacePrefs }
}
